                ...
                synchronized (lock) {
                    if (running.get()) {
                        try {
                            ...
                            BotLogger.error(global.getLocalizedMessage(), global);
                            try {
                                synchronized (lock) {
                                ...
                                BotLogger.debug(e.getLocalizedMessage(), e);
                                interrupt();
                            }
                        }
                    }
            ...
            try (CloseableHttpResponse response = httpclient.execute(httpPost, options.getHttpContext())) {
                ...
                if (response.getStatusLine().getStatusCode() >= 500) {
                    ...
                    try {
                        ...
                        BotLogger.error("Error deserializing update: " + responseContent, e);
                    }
                }
            } catch (SocketException | InvalidObjectException | TelegramApiRequestException e) {
                BotLogger.error(e.getLocalizedMessage(), e);
            } catch (SocketTimeoutException e) {
                BotLogger.info(e.getLocalizedMessage(), e);
            } catch (InterruptedException e) {
                BotLogger.info(e.getLocalizedMessage(), e);
                interrupt();
                ...
                if (e.getCause() instanceof InvocationTargetException) {
                    ...
                    BotLogger.error(cause.getLocalizedMessage(), cause);
                } else throw e;
            }
